...
export const run = async (input: Inputs): Promise<void> => {
  try {
    ...
    try {
      ...
      if (e instanceof Error) core.info(`Failed to summon monk: ${e.name} ${e.message} ${e.stack || 'No stack available'}`);
    ...
    if (err instanceof Error) core.error(`sumting wrong with something: ${err.name} ${err.message} ${err.stack || 'No stack available'}`);
